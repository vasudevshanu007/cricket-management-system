...
export const eventLoadAction = (pageNumber, keyword = '', cat = '', location = '') => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/api/events/show/?pageNumber=${pageNumber}&keyword=${keyword}&cat=${cat}&location=${location}`)
        dispatch({
...
export const eventLoadSingleAction = (id) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/api/event/${id}`);
        dispatch({
...
export const deleteSingleEventAction = (event_id) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.delete(`https://cricket-management-system-2.onrender.com/api/event/delete/${event_id}`);
        dispatch({
...
export const editSingleEventAction = (event) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.put(`https://cricket-management-system-2.onrender.com/api/event/update/${event._id}`, event);
        dispatch({
...
export const registerAeventAction = (event) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post("https://cricket-management-system-2.onrender.com/api/event/create", event)
        dispatch({
