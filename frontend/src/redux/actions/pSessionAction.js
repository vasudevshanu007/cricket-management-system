...
export const pSessionLoadAction = (pageNumber,keyword='',startDate='',endDate='',cat='')=>async(dispatch) => {
    ...
    try{
        const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/api/practicesessions/show?pageNumber=${pageNumber}&keyword=${keyword}&startDate=${startDate}&endDate=${endDate}&cat=${cat}`)
        dispatch({ 
...
export const singlepSessionLoadAction = (id)=>async(dispatch) => {
    ...
    try{
        const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/api/practicesession/${id}`);
        dispatch({ 
...
export const registerApSessionAction = (praticeSession) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post("https://cricket-management-system-2.onrender.com/api/practicesession/create", praticeSession)
        dispatch({
...
export const deleteSinglePsessionAction = (session_id) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.delete(`https://cricket-management-system-2.onrender.com/api/practicesessions/delete/${session_id}`);
        dispatch({
...
export const editSinglePsessionAction = (session) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.put(`https://cricket-management-system-2.onrender.com/api/practicesession/update/${session._id}`, session);
        dispatch({
