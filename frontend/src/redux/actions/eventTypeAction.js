...
export const eventTypeLoadAction = () => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get('https://cricket-management-system-2.onrender.com/api/type/events');
        dispatch({
...
export const createEventTypeAction = (eventtype) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post("https://cricket-management-system-2.onrender.com/api/type/create", eventtype)
        dispatch({
...
export const deleteSingleEventTypeAction = (type_id) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.delete(`https://cricket-management-system-2.onrender.com/api/type/delete/${type_id}`);
        dispatch({
