...
export const PerformanceLoadAction = (pageNumber, keyword = '',MainRole='') => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/api/performance/?pageNumber=${pageNumber}&keyword=${keyword}&MainRole=${MainRole}`)
        dispatch({
...
export const SinglePerformanceLoadAction = (ID) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/api/performance/${ID}`)
        dispatch({
...
export const PerformanceDeleteAction = (ID) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.delete(`https://cricket-management-system-2.onrender.com/api/admin/performance/delete/${ID}`)
        dispatch({
