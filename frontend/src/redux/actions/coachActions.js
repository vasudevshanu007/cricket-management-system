...
import {
  ALL_COACH_LOAD_FAIL,
  ALL_COACH_LOAD_REQUEST,
  ALL_COACH_LOAD_SUCCESS,
  COACH_CREATE_FAIL,
  ...
  SINGLE_COACH_LOAD_FAIL,
  SINGLE_COACH_LOAD_REQUEST,
  SINGLE_COACH_LOAD_SUCCESS,
} from "../constant/coachConstant";

export const allCoachAction = () => async (dispatch) => {
  dispatch({ type: ALL_COACH_LOAD_REQUEST });
  try {
    const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/coach/`);
    dispatch({
...
export const deleteCoachAction = (_id) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.delete(
      `https://cricket-management-system-2.onrender.com/coach/delete/${_id}`
    );
    dispatch({
...
export const coachUpdateAction = (coach) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.put(
      `https://cricket-management-system-2.onrender.com/coach/update/${coach._id}`,
      coach
    );
    dispatch({
...
export const singleCoachAction = (id) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get(`https://cricket-management-system-2.onrender.com/coach/get/${id}`);
    dispatch({
...
export const coachSignUpAction = (coach) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post("https://cricket-management-system-2.onrender.com/coach/add", coach);
